...
export class ChatServerService {
  ...
  async sendMessage(data: SendMessageDto): Promise<ConversationMessage> {
    ...
    if (
      !(await this.chatService.isChannelMember({
        userId: user.id,
    ...
    const membership = await this.chatService.getMembership({
      userId: user.id,
      channelId: data.channelId,
    });
    if (
      membership.isMuted ||
      (await this.chatService.userIsBannedFromChannel({
        userId: user.id,
        channelId: data.channelId,
      }))
    ) {
    ...
    if (channel.type === ChannelType.DIRECT) {
      ...
      if (
        (
          await this.prisma.userBlockedUser.findMany({
            where: {
              OR: [
  ...
  async banUserFromChannel(
    data: UserOnUserActionDto,
    login42: string,
  ): Promise<UserBannedFromChannel> {
    ...
    const otherMembership = await this.chatService.getMembership({
      userId: otherUser.id,
      channelId: data.channelId,
    });
    if (
      membership.role === MemberRole.MEMBER ||
      otherMembership.role === MemberRole.OWNER
    ) {
      throw new UnauthorizedException();
    }
  ...
  async upgradeUserRole(
    ...
    if (
      membership.role === MemberRole.MEMBER ||
      otherMembership.role !== MemberRole.MEMBER
    ) {
      throw new UnauthorizedException();
    }
    return await this.prisma.memberOfChannel.update({
  ...
  async downgradeUserRole(
    ...
    if (
      membership.role === MemberRole.MEMBER ||
      otherMembership.role !== MemberRole.ADMIN
    ) {
      throw new UnauthorizedException();
    }
    return await this.prisma.memberOfChannel.update({
  ...
  async leaveChannel(
    ...
    const user = await this.userPrisma.findUserByLogin42(login42);
    if (
      !(await this.chatService.isChannelMember({ userId: user.id, channelId }))
    ) {
      throw new BadRequestException();
    }
    const membership = await this.chatService.getMembership({
