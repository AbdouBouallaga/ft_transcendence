...
export class ChatService {
  ...
  async createRoom(data: CreateChannelDto, login42: string): Promise<Channel> {
    ...
    if (room.length > 0) throw new BadRequestException();
    let hash = null;
  ...
  async inviteUserToChannel(
    ...
    if (
      ...
      (await this.isBannedFromChannel({
        userId: user.id,
        channelId: channel.id,
      }))
    ) {
  ...
  async banUserFromChannel(data: BanUserDto): Promise<UserBannedFromChannel> {
    ...
    const userBannedFromChannel =
      await this.prisma.userBannedFromChannel.create({
        data: {
          channelId: channel.id,
          userId: otherUser.id,
          isBanned: true,
    ...
    await this.rmMemberFromChannel({
      userId: otherUser.id,
      channelId: channel.id,
    });
  ...
  async sendMessageToChannel(data: SendMessageDto): Promise<Message> {
    ...
    if (channel.type === ChannelType.DIRECT) {
      const otherUser = (
        await this.prisma.memberOfChannel.findFirst({
          where: {
            channelId: channel.id,
            userId: {
              not: user.id,
  ...
  async getFullChannelInfo(
    ...
    if (!(await this.isMemberOfChannel({ userId, channelId })))
      throw new UnauthorizedException();
    const admins = (
      await this.prisma.adminOfChannel.findMany({
        where: {
          channelId,
        },
        select: {
  ...
  async getPublicChannels(userId: number): Promise<ChannelInfo[]> {
    return (
      await this.prisma.userBannedFromChannel.findMany({
        where: {
          NOT: {
            userId,
          },
  ...
  async isBannedFromChannel(data: {
    userId: number;
    channelId: number;
  }): Promise<boolean> {
    const userBannedFromChannel =
    ...
    return (
      userBannedFromChannel.filter((element) => {
        return element.isBanned;
      }).length > 0
    );
  }

  async isMutedFromChannel(data: {
    userId: number;
    channelId: number;
  }): Promise<boolean> {
    const userBannedFromChannel =
    ...
    return (
      userBannedFromChannel.filter((element) => {
        return (
          new Date(element.startedAt.getTime() + element.duration * 60000) >=
          new Date()
        );
      }).length > 0
    );
