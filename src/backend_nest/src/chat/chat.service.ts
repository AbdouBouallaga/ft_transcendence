...
export class ChatService {
  ...
  async createRoom(data: CreateChannelDto, login42: string): Promise<Channel> {
    ...
    if (room.length > 0) throw new BadRequestException('name is already used!');
  ...
  async sendMessageToChannel(data: SendMessageDto): Promise<Message> {
    ...
    const channel = await this.findChannelById(data.channelId);
    if (
      !this.isMemberOfChannel({ userId: user.id, channelId: channel.id }) ||
  ...
  async isBannedFromChannel(data: {
    ...
    return (
      typeof userBannedFromChannel.find((element) => {
